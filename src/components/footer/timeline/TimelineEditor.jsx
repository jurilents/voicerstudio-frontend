...
const TimelineEditor = ({headingRef}) => {
  ...
  const addSubtitle = useCallback(
    (param) => {
      ...
      if (!selectedAction) {
        const startTime = +(Math.round(param.time * 10) / 10).toFixed(3);
  ...
  function setSubStartEndTime({action, dir, start, end}) {
    if (dir === 'left') {
      dispatch(patchSub(action, {start: +start.toFixed(3)}));
    } else if (dir === 'right') {
      dispatch(patchSub(action, {end: +end.toFixed(3)}));
    } else {
      dispatch(patchSub(action, {start: +start.toFixed(3), end: +end.toFixed(3)}));
    }
  }
