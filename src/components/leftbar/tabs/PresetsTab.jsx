...
const PresetsTab = () => {
  ...
  return (
    <Style className="tab-outlet">
      <div className="mb-5">
        <h3>{t('tabs.presets.voicingBots')}</h3>
        <Container>
          <Row>
            {azureCreds.length > 0 ? (
              <>
              ...
              <Col xs={12} className="text-center mb-2">
                {t('tabs.presets.pressToAddCredentials')}
              </Col>
            )}
          </Row>
        </Container>
          ...
          {credentials.map((cred) => (
            <ListGroup.Item key={cred.value} className={isCredSelected(cred) ? 'preset-selected' : ''}>
              ...
              <input className="list-item-input"
                     ...
                     onChange={(event) => dispatch(patchCreds(cred, {displayName: event.target.value}))}/>
              <span className="list-item-actions">
                <button className="btn" onClick={() => dispatch(removeCreds(cred))}>
        ...
        <ListGroup className="presets-list app-list-group-bottom">
          <ListGroup.Item>
            <button className="btn add-button" onClick={() => toggleCredsModal(true)}>
      ...
      <div>
        ...
        <ListGroup className="presets-list app-list-group">
          {presets.map((preset) => (
            <ListGroup.Item key={preset.id}>
              ...
              <span className="list-item-actions">
                <button className="btn" onClick={() => dispatch(removePreset(preset.id))}>
                  <FontAwesomeIcon icon={faTrashAlt}/>
                </button>
              </span>
            </ListGroup.Item>
        ...
        <ListGroup className="presets-list app-list-group-bottom">
          <ListGroup.Item>
            <button className="btn add-button" onClick={showAddPresetModal}>
              <FontAwesomeIcon icon={faAdd}/>
            </button>
          </ListGroup.Item>
        </ListGroup>
...
export default memo(PresetsTab, (prevProps, nextProps) => true);
