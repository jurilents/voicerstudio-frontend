...
function AudioVolumeItem({title, propertyName, type, player}) {
  ...
  return (
    <div className={type ? 'volume-wrapper ' + type : 'volume-wrapper'}>
      ...
      <div>{Math.round(settings[propertyName] * 100)}%</div>
      <label>{title}</label>
      {type === 'master' ? (
...
function SpeakerAudioVolumeItem({speaker, state, setState}) {
  ...
  return (
    <div style={{'--c-speaker': speaker.color}} className={'speaker-volume volume-wrapper'}>
      <div className="input-wrapper">
        <input type="range"
               orient="vertical"
               min={0}
      ...
      <div>{Math.round(speaker.volume * 100)}%</div>
...
export default function MixerTab() {
  ...
  return (
    <Style className="mixer">
      <h3>{t('tabs.audioMixer.title')}</h3>
      <div className="mixer-content">
        <AudioVolumeItem title={t('tabs.audioMixer.master')} propertyName="masterVolume" type="master" player={player}/>
        <AudioVolumeItem title={t('tabs.audioMixer.original')} propertyName="originalVolume" player={player}/>
