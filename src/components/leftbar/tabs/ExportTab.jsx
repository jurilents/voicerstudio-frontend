...
export default function ExportTab() {
  ...
  const downloadSub = useCallback(
    (type) => {
      try {
        ...
        toast.success(
          <Trans i18nKey="tabs.export.exportFileSucceeded" values={{fileName}} components={{bold: <b/>}}/>,
        );
      } catch (e) {
        toast.error(`${t('tabs.export.exportFileFailed')}: ${e}`);
      }
    },
    [selectedSpeaker],
  );
  ...
  return (
    <Style className="tab-outlet">
      <div>
        <h3>{t('tabs.export.title')}</h3>
        <Container>
          <Row>
            <Col className="label">{t('tabs.export.speaker')}</Col>
            <Col>
              <Form.Select
                className="app-select"
                onChange={(event) => dispatch(selectSpeaker(+event.target.value))}
          ...
          <Row>
            <Col className="label">{t('tabs.export.format')}</Col>
            <Col>
              <Form.Select
                className="app-select"
                onChange={(event) => dispatch(setSettings({exportFormat: event.target.value}))}>
          ...
          <Row>
            <Col className="label">{t('tabs.export.codec')}</Col>
            <Col>
              <Form.Select
                className="app-select"
                onChange={(event) => dispatch(setSettings({exportCodec: event.target.value}))}
                value={exportCodec}>
          ...
          <Row>
            <Col>
              <input
                className="app-input"
                type="text"
                onChange={(event) => dispatch(setSettings({exportFileName: event.target.value}))}
                value={exportFileName}/>
            </Col>
          </Row>
          <Row>
            <Col>{t('tabs.export.speakerName')}</Col>
            <Col>
              <span className="pattern-key">{'{S}'}</span>
            </Col>
          </Row>
          <Row>
            <Col>{t('tabs.export.speakerLanguage')}</Col>
            <Col>
              <span className="pattern-key">{'{L}'}</span>
            </Col>
          </Row>
          <Row>
            <Col>{t('tabs.export.exportFormat')}</Col>
            <Col>
              <span className="pattern-key">{'{F}'}</span>
            </Col>
          </Row>
          <Row>
            <Col>{t('tabs.export.currentDate')}</Col>
            <Col>
              <span className="pattern-key">{'{D}'}</span>
            </Col>
          </Row>
          <Row>
            <Col>{t('tabs.export.currentTime')}</Col>
            <Col>
              <span className="pattern-key">{'{T}'}</span>
            </Col>
          </Row>
          <Row className="mt-4">
            <Col>
              <button className="btn btn-primary" onClick={() => generateAndExport(setIsPending)}>
                {t('tabs.export.exportAs')} {exportFormat}
              </button>
            </Col>
          </Row>
          <Row className="mt-4">
