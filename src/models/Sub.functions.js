...
export function canSubBeVoiced(sub) {
    ...
    return status === VoicedStatuses.none || status === VoicedStatuses.obsolete || status === VoicedStatuses.processing;
}
...
export function allSubsAreValid(subs) {
    for (const sub of subs) {
        if (!(sub.start >= 0 && sub.end >= 0 && sub.start < sub.end && !sub.invalidStart && !sub.invalidEnd)) {
            return false;
        }
    }
